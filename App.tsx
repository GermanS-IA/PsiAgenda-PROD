...
const App: React.FC = () => {
  ...
  const filterByDate = (date: string) => {
    const filtered = appointments.filter(a => a.FECHA_INICIO === date)
  ...
  const handleExportJSON = () => {
      ...
      // We clear the warning immediately for UX, assuming the user completed the share flow
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const confirmed = window.confirm("ATENCIÓN: Al importar un respaldo, se reemplazarán todos los datos actuales por los del archivo. ¿Deseas continuar?");
